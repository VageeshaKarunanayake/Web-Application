...
$id=$_GET['mod'];
$cmd = $conn->prepare("SELECT id,name,student_count FROM groups WHERE course = '$id' AND prorata = '0'");
$cmd->execute();
...
$pdf->AddPage('L','','');
$pdf->SetFont('Arial','B',18);
$pdf->Write(5,' GROUPS STUDENT LIST');
	$pdf->Ln();
	$pdf->Ln();
$pdf->SetFont('Arial','B',9);
...
$pdf->Output('D','group_list.pdf','false');
